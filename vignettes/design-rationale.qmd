...
`cetype_ext_t` is complete for string data: it represents every encoding state R uses for `CHARSXP`s, along with `CE_ASCII`, `CE_ASCII_OR_UTF8`, and `CE_NA`. The two omitted `cetype_t` values, `CE_SYMBOL` and `CE_ANY`, are unused as encoding states for `CHARSXP`s.
