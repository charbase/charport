...
Call `to_sexp()` on the main R thread when the vector is done building: the `Builder` gives up its storage to the returned ALTREP `SEXP`. The terminal conversion uses the conventional ALTREP ownership order: it creates the external pointer, registers its finalizer, and then creates the ALTREP object. See [Error Handling](error-handling.qmd) for the distinction between this transfer and protected `Reader` acquisition.
...
The record table grows geometrically as elements are appended. `release_store()` moves those arrays into the finished `Store`; it does not allocate and repack a second metadata table. Payload slices start from the first payload request instead of using an assumed output length, then follow the normal slice growth policy.
...
The installed headers require C++11. They also compile as C++14, C++17, C++20, and C++23, so a consuming package may use a newer standard. Where it improves diagnostics without changing the C++11 API, charport enables newer language features conditionally. For example, writable reserve functions and the raw `resolve()` lease are `[[nodiscard]]` under C++17 and later.
