...
Every C++ function called through `.Call`, and every C++ `init` callback registered with `charport`, is entered through a C interface. Rcpp and cpp11 normally provide this exception boundary for `.Call` entry points generated by those frameworks. Hand-written `.Call` entry points and callbacks registered directly with `charport` must provide it themselves: if the code can throw a C++ exception, catch it before returning through the C interface. Copy the exception message, leave the `catch` block so C++ unwinding finishes, and only then call `Rf_error()` on the main R thread.

Reader access and `release(state)` callbacks have a stricter contract: for valid requests they must not call the R API and must not throw. This is required because access callbacks may run on worker threads.
