...
# resolve loop, and cp::charvec::Builder/BuilderShard must reproduce any
...
for (k in c(0L, 1L, 2L, 3L, 8L)) {
  ...
  s <- stats(out)
  stopifnot(s$allocated_bytes - s$dead_bytes == s$allocated_bytes)  # dead == 0
...
catn("builder accepts the direct path's policy-clean views (ascii/bytes/NA)")
plain <- c("abc", NA, "", b, "zz")
...
catn("set throws on policy-violating views (latin1 via the direct path)")
latin1_word <- w_latin1[which(Encoding(w_latin1) == "latin1")[1L]]
expect_error_matching(rebuild(c("a", latin1_word), 1L), "charport.*encoding")
expect_error_matching(rebuild(c("a", latin1_word), 0L), "charport.*encoding")

catn("builder error contract (throwing set, single-shot finish, safe abandon)")
...
stopifnot(all(is.na(out)), stats(out)$allocated_bytes == 0)
